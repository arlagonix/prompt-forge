...
var FIELD_TYPES = ["textarea", "text", "number", "date", "checkbox", "select", "combobox", "radio"];
...
function getDefinitionByName(group, name) {
  var _a;
  return (_a = group.children.find(function (child) {
...
function getFieldDefinitionByName(group, name) {
  ...
  return (found === null || found === void 0 ? void 0 : found.kind) === "field"
    ? found
    : null;
}
...
function getGroupDefinitionByName(group, name) {
  var found = getDefinitionByName(group, name);
  return (found === null || found === void 0 ? void 0 : found.kind) === "group"
    ? found
    : null;
}
...
function ensureUniqueChildName(group, name) {
  if (getDefinitionByName(group, name)) {
    throw new Error(
      'Duplicate name "'.concat(name, '" in scope "').concat(group.name, '".'),
    );
  }
}

function normalizeMetadataParam(raw, ancestorGroupNames) {
  if (ancestorGroupNames === void 0) {
    ancestorGroupNames = [];
  }
  ...
  if (item.type === "group") {
    if (ancestorGroupNames.includes(name)) {
      throw new Error(
        'Group name "'.concat(name, '" must be unique along the nesting path.'),
      );
    }
    ...
    for (
      var _i = 0, rawChildren_1 = rawChildren;
      _i < rawChildren_1.length;
      _i++
    ) {
      var rawChild = rawChildren_1[_i];
      var normalizedChild = normalizeMetadataParam(
        rawChild,
        __spreadArray(
          __spreadArray([], ancestorGroupNames, true),
          [name],
          false,
        ),
      );
...
function resolveFieldReference(scopeStack, name) {
  ...
  if (getGroupDefinitionByName(currentScope, name)) {
    throw new Error(
      'Placeholder "'
        .concat(name, '" conflicts with group "')
        .concat(name, '" in scope "')
        .concat(currentScope.name, '".'),
    );
  }
...
function parseTemplate(content) {
  ...
  var _a = parseFrontMatter(content),
  ...
  for (
    var _i = 0, metadataParamsRaw_1 = metadataParamsRaw;
    _i < metadataParamsRaw_1.length;
    _i++
  ) {
  ...
  while (cursor < body.length) {
    ...
    if (groupStartMatch) {
      ...
      if (!childGroup) {
        throw new Error(
          'Group "'
            .concat(groupName, '" is not declared in scope "')
            .concat(currentScope.name, '".'),
        );
      }
  ...
  if (openGroupNodeStack.length > 0) {
    throw new Error(
      'Group "'.concat(
        openGroupNodeStack[openGroupNodeStack.length - 1].name,
        '" was not closed.',
      ),
    );
  }
  ...
  for (var _i = 0, _b = group.renderOrder; _i < _b.length; _i++) {
    ...
    if (item.kind === "field") {
      fields[item.field.name] =
        (_a = item.field.defaultValue) !== null && _a !== void 0 ? _a : "";
      continue;
    }
...
function hasNonEmptySegmentText(segments) {
  return segments.some(function (segment) {
    return segment.text.length > 0;
  });
}

function splitRepeatGroupNodes(nodes) {
  if (nodes.length === 0) {
...
function buildSegmentsFromNodes(nodes, scopeStack) {
  ...
  var _loop_1 = function (node) {
    ...
    var currentScope = scopeStack[scopeStack.length - 1];
    var instances =
      (_b = currentScope.groups[node.definition.name]) !== null && _b !== void 0
        ? _b
        : [];

    var groupSegments = [];
    var renderedInstanceCount = 0;
    var repeatParts = node.definition.repeat
      ? splitRepeatGroupNodes(node.children)
      : { itemNodes: node.children, separatorText: "" };

    instances.forEach(function (instance) {
      var instanceSegments = buildSegmentsFromNodes(
        repeatParts.itemNodes,
        __spreadArray(__spreadArray([], scopeStack, true), [instance], false),
      );
...
function buildPromptSegments(bodyContent, content, formValues) {
  ...
  var tmpl =
    (_a =
      bodyContent !== null && bodyContent !== void 0
        ? bodyContent
        : content) !== null && _a !== void 0
      ? _a
      : "";
...
function stripReusableFlag(content) {
  ...
  var _a = parseFrontMatter(content),
    metadata = _a.metadata,
    body = _a.body,
    hasFrontMatter = _a.hasFrontMatter;
